...
function (app, FauxtonAPI, CORS) {
  ...
  Views.OriginDomainTable = FauxtonAPI.View.extend({
    ...
    beforeRender: function () {
      var origins = this.model.get('value');
    ...
    showRows: function () {
        var originsArray = this.model.get('value').split(',');
  ...
  Views.OriginDomainRow = FauxtonAPI.View.extend({
    ...
    serialize: function () {
      return {
        url: this.model.get('value').split(',')[this.index]
      };
    },
    ...
    onSaveDomain: function () {
      ...
      var domains = this.model.get('value').split(',');
    ...
    saveOrigins: function (origins) {
      var options = {section: 'cors', name: 'origins', value: origins.toString()};
  ...
  Views.CORSMain = FauxtonAPI.View.extend({
    ...
    beforeRender: function() {
      ...
      this.collection.each(function(config, index) {
          
        if (config.get("name") == 'origins') {
            that.origins = config.get('value');
            var tableModel = new CORS.ConfigModel({
              section: 'cors',
              name: config.get('name'),
              value: config.get('value')
            });
            
            that.originDomainTable = that.setView('.origin-domains', new Views.OriginDomainTable({ collection: collection, model: tableModel }));       
        }
        
        if (config.get("name") == 'credentials') {
            this.enableCors = config.get("value");
        }
      });
    },
    ...
    submit: function(e){
      ...
      if (data.enable_cors === 'on') {
        ...
        if (data.restrict_origin_domains === 'on') {
          var storedOrigins = this.origins.split(',');
