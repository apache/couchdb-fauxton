...
export const initDiffEditor = (dbName, docId) => dispatch => {
  ...
  Promise.all([db.get(docId), getTree(db, docId)]).then(([doc, tree]) => {
    ...
    db.get(doc._id, {rev: initialRev}).then(conflictDoc => {
...
export const chooseLeaves = (doc, revTheirs) => dispatch => {
  db.get(doc._id, {rev: revTheirs}).then(res => {
...
export const selectRevAsWinner = (databaseName, docId, paths, revToWin) => dispatch => {
  ...
  post(FauxtonAPI.urls('bulk_docs', 'server', databaseName, ''), payload).then(resp => {
    if (resp.error) {
