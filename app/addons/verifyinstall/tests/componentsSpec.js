...
define([
  ...
  describe('VerifyInstallResults', function () {
    ...
    it('confirm each result field shows success after successful test', function () {
      tests.forEach(function (test) {
        ...
        el = ReactTestUtils.renderIntoDocument(React.createElement(Components.VerifyInstallResults, {testResults: copy}), container);
  ...
  describe('VerifyInstallButton', function () {
    ...
    it('calls verify function on click', function () {
      ...
      el = ReactTestUtils.renderIntoDocument(React.createElement(Components.VerifyInstallButton, {verify: stub.func, isVerifying: false}), container);
    ...
    it('does not call verify function when verification already ongoing', function () {
      var stub = { func: function () { } };
      var spy = sinon.spy(stub, 'func');
      el = ReactTestUtils.renderIntoDocument(React.createElement(Components.VerifyInstallButton, {verify: stub.func, isVerifying: true}), container);
      ReactTestUtils.Simulate.click($(el.getDOMNode())[0]);
